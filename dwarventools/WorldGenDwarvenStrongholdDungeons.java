...
public class WorldGenDwarvenStrongholdDungeons extends WorldGenerator
{
		...
		for (x = xCoord - xRange - 1; x <= xCoord + xRange + 1; ++x)
		{
			for (y = yCoord - 1; y <= yCoord + yRange + 1; ++y)
			{
				for (z = zCoord - zRange - 1; z <= zCoord + zRange + 1; ++z)
				{
					...
					if (y == yCoord - 1 && !material.isSolid())
					{
						return false;
					}
					
					if (y == yCoord + yRange + 1 && !material.isSolid())
					{
						return false;
					}
					...
					for (z = zCoord - zRange - 1; z <= zCoord + zRange + 1; ++z)
					{
						...
						else if (world.getBlockMaterial(x, y, z).isSolid())
						{
							if (y == yCoord - 1 && random.nextInt(4) != 0)
							{
								...
								world.setBlockMetadataWithNotify(x, y, z, random.nextInt(3));
							}
							else
							{
								world.setBlockWithNotify(x, y, z, Block.stoneBrick.blockID);
